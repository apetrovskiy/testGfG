...
def is_later(period1: List[int], period2: List[int]) -> bool:
    return period1[0] > period2[0] or (
        period1[0] == period2[0] and period1[1] > period2[1]
    )
...
def is_interval_in(
    interval: List[List[int]], interval_to_check: List[List[int]]
) -> bool:
    return is_time_in(interval, interval_to_check[0]) or is_time_in(
        interval, interval_to_check[1]
    )


def is_interval_in_schedule(
    schedule: List[List[List[int]]], interval_to_check: List[List[int]]
) -> bool:
    if len(schedule) == 0:
        return False
