from src.main.java.problems.medium.MinimumPlatforms.minimum_platforms_shorter import (
    minimumPlatform,
)
from typing import List
from src.main.java.problems.medium.MinimumPlatforms.minimum_platforms import (
    is_earlier,
    is_interval_in,
    is_interval_in_schedule,
    is_time_in,
    is_later,
    minimumPlatformSlow,
)
...
@pytest.mark.parametrize(
    "number,arrival_times,departure_times,expected_result", test_data
)
...
def test_minimum_platforms_shorter(
    number: int,
    arrival_times: List[str],
    departure_times: List[str],
    expected_result: int,
):
    assert expected_result == minimumPlatform(number, arrival_times, departure_times)
...
@pytest.mark.parametrize(
    "interval,interval_to_test,expected_result", test_data_is_interval_in
)
def test_is_interval_in(
    interval: List[List[int]], interval_to_test: List[List[int]], expected_result: bool
):
...
@pytest.mark.parametrize(
    "schedule,interval_to_test,expected_result", test_data_is_interval_in_schedule
)
def test_is_interval_in_schedule(
    schedule: List[List[List[int]]],
    interval_to_test: List[List[int]],
    expected_result: bool,
):
    assert expected_result == is_interval_in_schedule(schedule, interval_to_test)
